...
/**
 * Truthful, observation-only DJI console projection.
 *
 * Public runtime lock truth remains LOCKED throughout hardware commissioning. The personalized
 * commissioning authority view is owned by ConsoleServerCore and never changes this snapshot.
 */
class DjiConsoleSnapshotProvider internal constructor(
    private val connectionState: () -> ConnectionState,
    private val latestAgentTelemetry: () -> Telemetry?,
    ...
    constructor(
        ...
        latestAgentTelemetry = agent.telemetry::latest,
        capabilitySnapshotLoader = capabilitySnapshotLoader,
    ...
    override fun runtimeState(): RuntimeStatePayload =
        RuntimeStatePayload(
            ...
            aircraftConnection = connectionState().toProtocol(),
    ...
    override fun latestTelemetry(): TelemetryPayload? = latestAgentTelemetry()?.toProtocol()

    internal fun mapTelemetry(telemetry: Telemetry): TelemetryPayload = telemetry.toProtocol()
    ...
    private fun Telemetry.toProtocol(): TelemetryPayload =
        TelemetryPayload(
            ...
            flightState = flightState?.mode.toProtocol(),
