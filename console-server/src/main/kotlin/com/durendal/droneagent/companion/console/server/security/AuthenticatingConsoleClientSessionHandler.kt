...
internal class AuthenticatingConsoleClientSessionHandler(
    ...
    override fun onProtocolSelected(
        ...
        synchronized(session.lock) {
            if (session.closeReason != null || session.phase != SessionPhase.REGISTERED) {
                throw ConsoleSessionProtocolException(ProtocolErrorCode.SERVER_UNAVAILABLE)
            }
            check(session.selectedProtocolVersion == null) {
                "console protocol version is already selected"
            }
    ...
    private fun authenticateOrRefuse(
        ...
        val selectedProtocolVersion =
            synchronized(session.lock) {
                checkNotNull(session.selectedProtocolVersion) {
                    "transport protocol selection is missing"
                }
            }
        inner.onSessionOpened(sessionId)
        inner.onProtocolSelected(sessionId, selectedProtocolVersion)
        val mayForwardHello =
            synchronized(session.lock) {
